  ...
  const char sst5[17] = {161, 181, 130, 140, 32, 191, 128, 190, 191, 176, 187, 176, 33 ,0}; //Сеть пропала! (autogenerated by ru_str_converter.py script)
	...
	uint8_t sst5[19] = "Power off!        ";
	 ...
	 if(al->BatteryOut
		...
		if(al->BatteryOut) { u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst0); line_num++;}
		if(al->ConcentratorMax) {u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst1);line_num++;}
		if(al->CylindersEmpty) {u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst2);line_num++;}
		if(al->LineMax) {u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst3);line_num++;}
		if(al->LineMin) {u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst4);line_num++;}
		if(al->PowerOff) {u8g_DrawStr(&ffont, x + thickness + 2, y + line_num * step + thickness + offset_err, sst5);line_num++;}		
#endif
